...
public:
	DeviceProxy() {}
	virtual ~DeviceProxy() { OSDebugOut(TEXT("%p\n"), this); }
	virtual USBDevice* CreateDevice(int port) { return T::CreateDevice(port); }
	virtual const TCHAR* Name() const { return T::Name(); }
	virtual const char* TypeName() const { return T::TypeName(); }
	virtual int Configure(int port, const std::string& api, void* data)
	{
		return T::Configure(port, api, data);
	}
	virtual std::list<std::string> ListAPIs() { return T::ListAPIs(); }
...
public:
	...
	void Clear() { registerProxyMap.clear(); }
	...
	T* Proxy(const std::string& name) { return registerProxyMap[name].get(); }

	std::list<std::string> Names() const
	{
		std::list<std::string> nameList;
		std::transform(registerProxyMap.begin(), registerProxyMap.end(),
					   std::back_inserter(nameList), SelectKey());
		return nameList;
	}
	...
	const RegisterProxyMap& Map() const { return registerProxyMap; }
...
public:
	typedef std::map<DeviceType, std::unique_ptr<DeviceProxyBase>>
		RegisterDeviceMap;
    ...
    return nullptr;*/
		auto proxy =
			std::find_if(registerDeviceMap.begin(), registerDeviceMap.end(),
	...
	{
		std::list<std::string> nameList;
		std::transform(registerDeviceMap.begin(), registerDeviceMap.end(),
					   std::back_inserter(nameList), SelectDeviceName());
		return nameList;
	}
	...
	const RegisterDeviceMap& Map() const { return registerDeviceMap; }
