...
namespace usb_mic
{
	...
	public:
		...
		virtual AudioDevice*
		CreateObject(int port, const char* dev_type, int mic,
					 AudioDir dir) const = 0; // Can be generalized? Probably not
		virtual void AudioDevices(std::vector<AudioDeviceInfo>& devices,
								  AudioDir) const = 0;
	...
	public:
		...
		AudioDevice* CreateObject(int port, const char* dev_type, int mic,
								  AudioDir dir) const
		{
			try
			{
		...
		virtual bool AudioInit() { return T::AudioInit(); }
		virtual void AudioDeinit() { T::AudioDeinit(); }
	};
