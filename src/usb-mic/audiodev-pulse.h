...
#define APINAME_ "pulse"
		...
		public:
				...
				, mOutSamples(0)
			{
				...
				const char* var_names[] = {N_AUDIO_SOURCE0, N_AUDIO_SOURCE1, N_AUDIO_SINK0,
										   N_AUDIO_SINK1};

				if (!LoadSetting(mDevType, mPort, APINAME,
								 (device ? var_names[i + 1] : var_names[i]), mDeviceName) ||
					mDeviceName.empty())
					throw AudioDeviceError(APINAME_ ": failed to load device settings");

				LoadSetting(mDevType, mPort, APINAME,
							(dir == AUDIODIR_SOURCE ? N_BUFFER_LEN_SRC : N_BUFFER_LEN_SINK),
							mBuffering);
			...
			inline uint32_t GetChannels() { return mSSpec.channels; }

			static const char* TypeName() { return APINAME; }

			static const TCHAR* Name() { return "PulseAudio"; }
