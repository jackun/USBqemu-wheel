                   ...
                   uint8_t *dest, size_t len)
{
    ...
    d->u.interface_.bInterfaceNumber   = iface->bInterfaceNumber;
    d->u.interface_.bAlternateSetting  = iface->bAlternateSetting;
    d->u.interface_.bNumEndpoints      = iface->bNumEndpoints;
    d->u.interface_.bInterfaceClass    = iface->bInterfaceClass;
    d->u.interface_.bInterfaceSubClass = iface->bInterfaceSubClass;
    d->u.interface_.bInterfaceProtocol = iface->bInterfaceProtocol;
    d->u.interface_.iInterface         = iface->iInterface;
    pos += bLength;
...
void usb_desc_set_string(USBDevice *dev, uint8_t index, const char *str)
{
    ...
    if (s == NULL) {
        s = (USBDescString*)malloc(sizeof(*s));
        s->index = index;
...
void usb_desc_create_serial(USBDevice *dev)
{
	...
	//TODO 
	usb_desc_set_string(dev, index, "1234");
