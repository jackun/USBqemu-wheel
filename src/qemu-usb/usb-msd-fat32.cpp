...
struct BIOSParameterBlock
{
	...
	uint8_t sectors_per_cluster; // default to 8 maybe. 2^n where <= 32K bytes per cluster
	uint16_t reserved_sectors; // 32
	uint8_t num_fats; // 1 or 2
	uint16_t root_entries; // 0 on FAT32
	uint16_t total_sectors16; // < 65535 sectors, set to 0 for (large) total_sectors
	uint8_t media_desc; //0xF8 - fixed
	uint16_t sectors_per_fat; //fat12/16 only
	uint16_t sectors_per_track;
	uint16_t num_heads;
	uint32_t hidden_sectors; //0 when non-partitioned
	uint32_t total_sectors;

	void SetDefaults()
	{
		memset(this, 0, sizeof(BIOSParameterBlock));
		strncpy(oem, "MSWIN4.1", ARRAYSIZE(oem));
		...
		total_sectors16 = 0;
		media_desc = 0xF8;
		sectors_per_fat = 0; //4033 - 256MB image
		sectors_per_track = 32;
		num_heads = 64;
		hidden_sectors = 0;
		total_sectors = 0x7FFFFFFF; // TODO 0x00080000 - 256MB
	}
};
...
struct ExtendedBootRecord
{
	...
	uint8_t reserved[12];
	uint8_t drive_nr; //0x80 ?
	uint8_t flags_nt;
	uint8_t sig; //0x28 || 0x29 ?
	uint32_t vol_id;
	char vol_name[11];
	char vol_sig[8]; // "FAT32   "
	uint8_t boot_code[420];
	uint16_t boot_sig; //0xAA55

	void SetDefaults()
	{
		memset(this, 0, sizeof(ExtendedBootRecord));
		ext_flags = 0 | (1 << 7);
		root_dir_cluster = 2;
		fsinfo_sector = 1;
		backup_sector = 6; //usually 6
		drive_nr = 0x80;
		sig = 0x29; //extended
		vol_id = 0x12345678;
		strncpy(vol_name, "QEMU    ", ARRAYSIZE(vol_name));
		strncpy(vol_sig, "FAT32   ", ARRAYSIZE(vol_sig));
		//memset(boot_code, 0, sizeof(boot_code));
		boot_sig = 0xAA55;
	}
};
...
struct FSInfo
{
	...
	uint32_t free_cnt; // 0xFFFFFFFF for unknown
	uint32_t next_free; // 0xFFFFFFFF for unknown
...
public:
	...
	FAT32Table(size_t size)
	{
		bpb.SetDefaults();
		ebr.SetDefaults();
		fsinfo.SetDefaults();

		bpb.total_sectors = size / bpb.bytes_per_sector;
		size_t remaining = size - 1024 - bpb.hidden_sectors * bpb.bytes_per_sector;
		// GNU mtools formula
		size_t fatSize = std::ceil(float((remaining + 2 * bpb.sectors_per_cluster) * 8) / (2 * bpb.sectors_per_cluster * bpb.bytes_per_sector + bpb.num_fats * 8));
		bpb.sectors_per_fat = fatSize / bpb.bytes_per_sector;

		size_t fat_size = bpb.hidden_sectors * bpb.bytes_per_sector + fatSize /* * bpb.num_fats */;
		fat = new uint8_t[fat_size];
		end = fat + fat_size;
		backup_sect = ebr.backup_sector * bpb.bytes_per_sector;
	}

	bool Read(size_t offset, void *buffer, size_t len)
	{
		bool ret = true;
		if ((offset == 0 || offset == backup_sect))
		{
			size_t final_size = MIN(len, 512);
			memcpy(buffer, &bpb, final_size);
		}
		else if (offset == sizeof(BIOSParameterBlock) || offset == 0xC00 /*backup_sect + sizeof(BIOSParameterBlock)*/)
		{
			size_t final_size = MIN(len, 512);
			memcpy(buffer, &ebr, final_size);
		}
		return ret;
	}
...
USBDevice *MsdDeviceFAT32::CreateDevice(int port)
{
	...
	std::string api = *MsdDeviceFAT32::APIs().begin();
	...
	sprintf(s->dev.devname, "QEMU USB MSD(%.16s)",
			 ""/*filename*/);
	int i = sizeof(BIOSParameterBlock);
	usb_msd_handle_reset((USBDevice *)s);
	return (USBDevice *)s;
}

REGISTER_DEVICE(99, DEVICENAME, MsdDeviceFAT32);
#undef DPRINTF
#undef DEVICENAME
#undef APINAME
