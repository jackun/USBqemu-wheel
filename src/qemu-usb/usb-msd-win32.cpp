...
static const wchar_t entire_folder[] = { L"\\Entire folder" };
BOOL CALLBACK MsdDlgProcFAT32(HWND hW, UINT uMsg, WPARAM wParam, LPARAM lParam) {
	int port;
	static wchar_t buff[4096] = { 0 };
	...
	case WM_INITDIALOG:
	{
		...
		SetWindowLong(hW, GWL_USERDATA, (LONG)lParam);
		CONFIGVARIANT var(N_CONFIG_PATH, CONFIG_TYPE_WCHAR);
		if (LoadSetting(port, APINAME "_fat32", var))
			wcsncpy_s(buff, var.wstrValue.c_str(), ARRAYSIZE(buff));
		SetWindowTextW(GetDlgItem(hW, IDC_EDIT1), buff);
		return TRUE;
	}
	...
	case WM_COMMAND:
		...
		if (HIWORD(wParam) == BN_CLICKED) {
			...
			case IDC_BUTTON1:
				...
				wcsncat_s(buff, entire_folder, ARRAYSIZE(entire_folder));
				ZeroMemory(&ofn, sizeof(ofn));
				ofn.lStructSize = sizeof(ofn);
				ofn.hwndOwner = hW;
				ofn.lpstrTitle = L"USB image directory";
				ofn.lpstrFile = buff;
				ofn.nMaxFile = ARRAYSIZE(buff);
				ofn.lpstrFilter = nullptr;// L"All\0*.*\0";
				ofn.nFilterIndex = 1;
				ofn.lpstrFileTitle = NULL;
				ofn.nMaxFileTitle = 0;
				ofn.lpstrInitialDir = NULL;
			...
			case IDOK:
			{
				...
				CONFIGVARIANT var(N_CONFIG_PATH, CONFIG_TYPE_WCHAR);
				var.wstrValue = buff;
				if (!SaveSetting(port, APINAME "_fat32", var))
					res = RESULT_FAILED;
				//strcpy_s(conf.usb_img, ofn.lpstrFile);
				EndDialog(hW, res);
				return TRUE;
...
int MsdDeviceFAT32::Configure(int port, std::string api, void *data)
{
	Win32Handles handles = *(Win32Handles*)data;
	return DialogBoxParam(handles.hInst,
		MAKEINTRESOURCE(IDD_DLGMSD),
		handles.hWnd,
		(DLGPROC)MsdDlgProcFAT32, port);
}
#undef APINAME
