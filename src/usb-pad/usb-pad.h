...
#define N_JOYSTICK TEXT("joystick")
	...
	public:
		...
		static const TCHAR* Name() { return TEXT("Wheel device"); }
		static const char* TypeName() { return "pad"; }
	...
	public:
		...
		static USBDevice* CreateDevice(int port);
		static const TCHAR* Name() { return TEXT("Rock Band drum kit"); }
		static const char* TypeName() { return "rbdrumkit"; }
		static std::list<std::string> ListAPIs();
		static const TCHAR* LongAPIName(const std::string& name);
		static int Configure(int port, const std::string& api, void* data);
		static int Freeze(int mode, USBDevice* dev, void* data);
	...
	public:
		virtual ~BuzzDevice() {}
		static USBDevice* CreateDevice(int port);
		static const TCHAR* Name() { return TEXT("Buzz Device"); }
		static const char* TypeName() { return "buzz_device"; }
		static std::list<std::string> ListAPIs();
		static const TCHAR* LongAPIName(const std::string& name);
		static int Configure(int port, const std::string& api, void* data);
		static int Freeze(int mode, USBDevice* dev, void* data);
	...
	public:
		...
		static USBDevice* CreateDevice(int port);
		static const TCHAR* Name() { return TEXT("Sega Seamic"); }
		static const char* TypeName() { return "seamic"; }
		static std::list<std::string> ListAPIs();
		static const TCHAR* LongAPIName(const std::string& name);
		static int Configure(int port, const std::string& api, void* data);
		static int Freeze(int mode, USBDevice* dev, void* data);
	...
	public:
		...
		static USBDevice* CreateDevice(int port);
		static const TCHAR* Name() { return TEXT("Keyboardmania"); }
		static const char* TypeName() { return "keyboardmania"; }
		static std::list<std::string> ListAPIs();
		static const TCHAR* LongAPIName(const std::string& name);
		static int Configure(int port, const std::string& api, void* data);
		static int Freeze(int mode, USBDevice* dev, void* data);
...
#define GENERIC_PID                                                              \
	0xC294 // actually Driving Force aka PID that most logitech wheels initially \
		   // report
...
#define MAX_JOYS 32
  ...
  Every Logitech wheel reports itself as generic Logitech Driving Force wheel
(VID 046d, PID c294). This is done to ensure that the wheel will work on every
USB HID-aware system even when no Logitech driver is available. It however
limits the capabilities of the wheel - range is limited to 200 degrees, G25/G27
don't report the clutch pedal and there is only one combined axis for throttle
and brake. The switch to native mode is done via hardware-specific command which
is different for each wheel. When the wheel receives such command, it simulates
reconnect and reports to the OS with its actual PID. Currently not emulating
reattachment. Any games that expect to?
**/
...
#define DESC_CONFIG_WORD(a) (a & 0xFF), ((a >> 8) & 0xFF)
	...
	static const uint8_t df_config_descriptor[] = {
		...
		DESC_CONFIG_WORD(
			sizeof(pad_driving_force_hid_report_descriptor)), // Size of the report
															  // descriptor
	...
	static const uint8_t gtforce_config_descriptor[] = {
		...
		1,                        // Number of class descriptors, see usbcfg.h
		USB_DT_REPORT,            // Report descriptor type
		DESC_CONFIG_WORD(sizeof(
			pad_gtforce_hid_report_descriptor)), // Size of the report descriptor
	...
	static const uint8_t kbm_hid_report_descriptor[] = {
		...
		0x19,
		0x3A, //   USAGE_MINIMUM (Button 58)
		0x29,
		0x3F, //   USAGE_MAXIMUM (Button 63)
		0x15,
		...
		0x81,
		0x02, //   INPUT
			  //   (Data,Variable,Absolute,NoWrap,Linear,PrefState,NoNull,NonVolatile,Bitmap)
		0x75,
		0x01, //   REPORT_SIZE (1)
		0x95,
		0x02, //   REPORT_COUNT (2)
		...
		0x09, //   USAGE_PAGE (Button)
		0x19,
		0x08, //   USAGE_MINIMUM (Button 8)
		0x29,
		0x0E, //   USAGE_MAXIMUM (Button 14)
		0x15,
		0x00, //   LOGICAL_MINIMUM (0)
		0x25,
		0x01, //   LOGICAL_MAXIMUM (1)
		...
		0x09, //   USAGE_PAGE (Button)
		0x19,
		0x0F, //   USAGE_MINIMUM (Button 15)
		0x29,
		0x15, //   USAGE_MAXIMUM (Button 21)
		0x15,
		0x00, //   LOGICAL_MINIMUM (0)
		0x25,
		0x01, //   LOGICAL_MAXIMUM (1)
		...
		0x09, //   USAGE_PAGE (Button)
		0x19,
		0x16, //   USAGE_MINIMUM (Button 22)
		0x29,
		0x1C, //   USAGE_MAXIMUM (Button 28)
		0x15,
		0x00, //   LOGICAL_MINIMUM (0)
		0x25,
		0x01, //   LOGICAL_MAXIMUM (1)
