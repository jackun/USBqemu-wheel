...
namespace usb_pad
{
	namespace dx
	{
		...
		static const DWORD PRECMULTI = 100; // floating point precision multiplier, 100
											// - two digit precision after comma
		...
		{
		public:
			DInputDevice(ControlType type, LPDIRECTINPUTDEVICE8 device, GUID guid,
						 TSTDSTRING name, uint32_t xinput = -1)
				: m_type(type)
				, m_guid(guid)
				, m_product(name)
				, m_device(device)
			...
			/*void GetDeviceState(size_t sz, void *ptr)
  {
          if (sz == sizeof(DIJOYSTATE2) && ptr)
			...
			LPDIRECTINPUTDEVICE8 GetDevice() { return m_device; }
