 ...
 *
 * input values 127 and 128 are center positions and are translated to output
 * value 0
 */
...
int16_t ff_lg_get_condition_coef(uint8_t caps, unsigned char k, unsigned char s,
								 int16_t max = SHRT_MAX);
uint16_t ff_lg_get_spring_deadband(uint8_t caps, unsigned char d,
								   unsigned char dL, uint16_t max = USHRT_MAX);
